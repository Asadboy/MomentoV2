...
- [ ] **In-app account deletion (Apple Guideline 5.1.1(v))** — currently users can sign out but not delete their account. Apple rejects apps that allow account creation without offering deletion. Add a "Delete my account" option in `ProfileView` that calls a Supabase RPC to soft-delete (or hard-delete) the user + their events + their photos + storage objects. Half-day of work; pretty much a guaranteed rejection cause if submitted without it. Surfaced by drafting `Docs/launch/APP_REVIEW_NOTES.md`.
